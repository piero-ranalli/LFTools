...
! Model Parameters
	...
	!dimensionality
      	integer sdim
      	!parameter(sdim=numparams)
      
       	! spriorran = s prior parameters
        ! it can be: 
        !     * range  aka parameter min,max  for flat priors
        !     * location,scale                for Cauchy priors
        ! values are set in lfmnconfig.f90
      	double precision, allocatable, target :: spriorpar(:,:)
        class(tprior), allocatable :: prior
...
! Parameters for MultiNest
	...
	!parameter(nest_rseed=-1)
        !parameter(nest_rseed=8321)
      	...
      	character*100 :: nest_root = 'chains/1-'
